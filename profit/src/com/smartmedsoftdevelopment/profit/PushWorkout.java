 ...
 */
public class PushWorkout extends Activity {
    int i=120;
    int setnumber = 0;
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        muscules.setImageDrawable(getResources().getDrawable(R.drawable.mpushup));
        ...
        int workoutnumber = pref.getInt("PUSH_WORKOUT_NUMBER",0);

        if(workoutnumber == 0)
        {
            SharedPreferences.Editor edit = pref.edit();
            edit.putInt("PUSH_WORKOUT_NUMBER", 1);
            edit.commit();
        }

        Cursor cursor = sdb.query("pushups",
                null,
                "_id = " + pref.getInt("PUSH_WORKOUT_NUMBER", 0), null,
                null, null, null);
...

    {
        ...
        if (setnumber < 5) {
            ...
            mCountDownTimer = new CountDownTimer(3000, 1000) {
                ...
                public void onFinish() {
                    ...
                    if (setnumber == 5) {
                        SharedPreferences.Editor edit = pref.edit();
                        if (pref.getInt("PUSH_WORKOUT_NUMBER", 0) < 18) {
                            edit.putInt("PUSH_WORKOUT_NUMBER", pref.getInt("PUSH_WORKOUT_NUMBER", 0) + 1);
                        }
                        else
                        {edit.putInt("PUSH_WORKOUT_NUMBER", pref.getInt("PUSH_WORKOUT_NUMBER", 0));}
                        edit.commit();
                        DataHelper mDbHelper = new DataHelper(context);
                        mDbHelper.openDataBase();
                        SQLiteDatabase sdb;
                        ...
                        sdb.insert("pushuphistory", null, values);
                        mDbHelper.close();
        ...
        {
            Intent intentGoToMain = new Intent(PushWorkout.this, DashboardActivity.class);
